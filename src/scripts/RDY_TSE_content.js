...
/* entrypoint for isCountersInEntryTab option
** 
** @param {bool} isDebugModeEnabled - extension parameter
** @returns {void}
*/
async function addSummaryByDayCountersToTimeentryTab(isDebugModeEnabled) {
  ...
  // // get time data
  // const time = await getTimeentryData(isDebugModeEnabled);
  ...
  // // get counters data
  // const counters = await getCountersData(isDebugModeEnabled);
  ...
  const initialNbOfColumns = tsFormTableHeader.childNodes.length-2; // minus first & last columns
  ...
  if(showWeekTotals && summary.weekly){
    addWeeklyCountersToTable(summary.weekly, tsFormTableBody, initialNbOfColumns, startdate, enddate, isDebugModeEnabled);
  }

  // add period counters to table body
  addPayPeriodCountersToTable(summary.timesheet, tsFormTableBody, initialNbOfColumns, isDebugModeEnabled);
}
